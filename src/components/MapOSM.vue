    ...
    onMounted( async ()=> {
      ...
      map = Leaflet.map('map',
        ...
        let myIcon = Leaflet.icon({
            iconUrl:    'src/assets/marker-icon-2x.png',       // Image de l'icone
            shadowUrl:  'src/assets/marker-shadow.png',     // Ombre de l'icone
        ...
        marker.bindPopup('TaVue magasin 1')
        ...
        // Ajouter une popup / infobulle
        marker2.bindPopup('TaVue magasin 2')
...
<template>
    <div class="container-fluid" style="color:black;">
        ...
        <div class="input-group">
            <button class="btn  bg-bouton hover:bg-bouton-hover">coordonnées : latitude</button> 
            <button class="btn bg-custom ">{{ coordMe.latitude}}</button> 
            <button class="btn  ml-6  bg-bouton hover:bg-bouton-hover">coordonnées : longitude</button> 
            <button class="btn bg-custom ">{{ coordMe.longitude}}</button> 
            <button class="btn bg-footer text-fond hover:bg-custom hover:text-footer ml-6" @click="locMe()">Se localiser</button> 
        </div>
